...
const ListingDetailPage = () => {
  ...
  const handleContactSeller = async () => {
    ...
    // 2. Prevent user from contacting themselves
    ...
    try {
      // 3. Send request to backend to start a conversation
      const res = await api.post(`/messages/start/${listing.seller._id}`, {
        listingId: listing._id,
      });
      // 4. Redirect to the messages page with the new conversation ID
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
      {/* Note: Corrected a typo here from md-col-span-2 to md:col-span-2 */}
