...
const BrowsePage = () => {
  ...
  const keyword = searchParams.get('keyword'); // <-- GET THE KEYWORD

  // This effect will re-run whenever a filter or keyword changes
  useEffect(() => {
    const fetchListings = async () => {
      ...
      try {
        ...
        if (keyword) params.append('keyword', keyword); // <-- ADD KEYWORD TO PARAMS
    ...
    fetchListings();
  }, [category, keyword, sort, maxPrice, selectedConditions]); // <-- ADD KEYWORD TO DEPENDENCY ARRAY
    ...
    <div className="flex gap-8">
      ...
      <main className="w-3/4">
        <h1 className="text-3xl font-bold text-brand-green mb-6 uppercase">
          {keyword ? `Results for "${keyword}"` : (category ? `${category}` : "Search Results")}
        </h1>
        {loading ? (
