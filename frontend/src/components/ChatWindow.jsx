...
const ChatWindow = () => {
    ...
    useEffect(() => {
        socket?.on("newMessage", (newMessage) => {
            // Add sound effect for new message if desired
            setMessages((prevMessages) => [...prevMessages, newMessage]);
        });
        return () => socket?.off("newMessage");
    }, [socket]);
    
    // Fetch historical messages when conversationId changes
    useEffect(() => {
        ...
        <>
            ...
            <div className="flex-grow p-6 overflow-y-auto">
                <div className="space-y-4">
                    {messages.map((msg) => {
                        // THIS IS THE FIX: Check msg.senderId._id against the logged-in user's ID
